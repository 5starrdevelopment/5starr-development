...
const BookNowPage = () => {
  ...
  // Fetch booked slots — reads from localStorage + Sheet API
  const fetchBookedSlots = async () => {
    setLoadingSlots(true);
    let allSlots = [];

    // Read from localStorage (coach-seeded bookings)
    try {
      const localData = localStorage.getItem("5starr-schedule");
      if (localData) {
        const localBookings = JSON.parse(localData);
        const approved = localBookings
          .filter(b => b.status === "approved" || b.status === "pending")
          .map(b => ({ date: b.date, time: b.time, duration: b.duration || 75, status: b.status }));
        allSlots = [...allSlots, ...approved];
      }
    } catch (e) { console.error("Local read error:", e); }

    // Also read from Sheet API if connected
    ...
    if (url) {
      try {
        ...
        if (data.success && data.bookings) {
          // Dedupe by date+time to avoid double-counting
          const localKeys = new Set(allSlots.map(s => s.date + s.time));
          const sheetNew = data.bookings.filter(b => !localKeys.has(b.date + b.time));
          allSlots = [...allSlots, ...sheetNew];
        }
